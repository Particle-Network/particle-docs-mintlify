...
For a detailed explanation of each feature and more advanced configurations, refer to the [API & SDK Introduction](/api-reference/introduction).
...
<video
  ...
  src="https://static.particle.network/mintlify/images/npm-connectkit.mp4"
></video>
...
<Step title="Setting Up a Connect Project">

Start by creating a boilerplate project with all the necessary dependencies:
...
The CLI will prompt you to enter a project name and select the framework you wish to use. 


```shell Terminal
🤩 Welcome to Particle Network!

✔ What is the name of your project? … particle-connectkit-app

? Which template would you like to use? › (Use arrow keys to navigate and press Enter to select)
❯   create-next-app
    ...
```

<Note>This guide will proceed with a Next.js project setup.</Note>
...
After obtaining your `projectId`, `clientKey`, and `appId`, configure **Particle Connect** in your app using the `ConnectKitProvider` component from `@particle-network/connectkit`. 

The boilerplate already includes the basic setup—just add your API keys to the `.env.sample` file and rename it to `.env`.

Then, move into your project directory and start the development server:

```sh Terminal
cd particle-connectkit-app && yarn dev
```

Start editing the `app/page.tsx` file to begin development.

<Tip>Continue with the following steps to learn how **Particle Connect** works and how to build your application.</Tip>


</Step>
<Step title="Configuring `ConnectKitProvider`">

It's recommended to create a `Connectkit.tsx` file in the `src` directory where you can configure and export the `ConnectKitProvider`. Then, use this provider to wrap your main App component.

Here’s how you can configure the `ConnectKitProvider`:

**Required Configurations:**
- `projectId`, `clientKey`, and `appId` — Get these from the [Particle dashboard](https://www.particle.network/dashboard).
...
const config = createConfig({
  ...
  plugins: [
    wallet({
      visible: true,
      entryPosition: EntryPosition.BR, // Use BR, BL, TR, TL to move the wallet entry position
    }),
  ],
...
Wrap your application with the `ParticleConnectKit` component (exported from `ConnectKitProvider`) in your index or layout file.

Here’s an example for a `layout.tsx` file:
...
With **Particle Connect** now configured, you can proceed to enable social logins within your application.

<Note>The **Connect** button can be directly imported from `@particle-network/connectkit`.</Note>

`@particle-network/connectkit` also provides a variety of hooks. You can explore all available hooks in the [Particle Connect SDK documentation](/api-reference/connect/desktop/web#key-react-hooks-for-particle-connect).

The boilerplate application includes a basic example featuring only a **Connect** button.

After logging in, users can access the embedded wallet modal provided by Particle Connect via the button in the bottom right corner, unless customized through the `wallet` configuration within `ConnectKitProvider`.

To explore additional features like fetching and displaying user information, balances, on-ramp services, and sending transactions using the native Particle Provider and ethers.js, paste the following code into `app/page.tsx`.
...
<Tip>
    The complete Particle Connect SDK reference is available at [Particle Connect SDK (Web)](/api-reference/connect/desktop/web).
    Additionally, a demo repository for Next.js can be found at [particle-connectkit2.0-quickstart](https://github.com/Particle-Network/particle-connectkit2.0-quickstart).
</Tip>
