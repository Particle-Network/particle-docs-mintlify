...
This document will go through the step-by-step process of configuring, initializing, and using Particle Connect on Flutter.
...
To begin, you'll need to begin by adding `particle_connect` to your Flutter application; this is a requirement before moving onto platform-specific configuration.
...
// inititilze ParticleConnect and ParticleAuthCore.
...
Basic account information can be found through the result of `ParticleConnect.connect`, in this case, that was saved to `account`, which contains properties such as `publicAddress` and `walletType`.
...
If you included `particl_wallet` within your `pubspec.yaml`, you can open the (optional) embedded wallet interface at any time after a user connects, use `ParticleWallet.setSupportChain` to set supported chains, then `ParticleWallet.navigatorWallet` to open the interface.
