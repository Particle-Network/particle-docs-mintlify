...
Generally, the local user state generated after a Particle Auth user logs in does not expire; however, if a user logs into the same app configuration (in the Particle Network backend, one project corresponds to multiple apps) in two different places (e.g., logging in successively in two browsers), only the latter login state will be valid. For this situation, there are the following solutions:
