...
description: "Customizing the appearance of Particle Auth's login modal."
---

**Particle Network** places a strong emphasis on **deep customization and application-specificity**. Whether it’s logos, colors, or border sharpness, virtually every aspect can be tailored programmatically using **Particle ConnectKit** or **Particle AuthKit**. This guide will explore the various methods available for customizing these tools to meet your specific needs.

***

## Particle Connect Customization

**Particle Connect** is the suggested SDK for integrating social logins and enabling Account Abstraction, and it allows deep programmatic customization when initializing the SDK with `createConfig` in the `ConnectKitProvider` component.
...
<Note>Each of these options allows you to customize the appearance of the **authentication modal**, **wallet modal**, and **transaction popups**.</Note>
...
The most common way to customize the Particle AuthKit interface is through programmatic style definitions, which can be applied during SDK initialization.
...
The `ThemeStyle` object allows you to customize the appearance of the **authentication menu** and **transaction popups**. It provides options for defining various visual elements, including colors, backgrounds, borders, and more.
...
The following example demonstrates how to implement a green hacker theme for the **Particle AuthKit** login modal:
