...
Particle Network has natively implemented session keys through both the [Account Abstraction RPC](/aa/rpc/getsmartaccount) and the AA SDKs ([Introduction to Smart Wallet-as-a-Service](/aa/introduction)), allowing developers to begin using session keys directly within the Particle Network tech stack.

***

## Creating a session key

Creating a session key within Particle Network's [Account Abstraction RPC](/aa/rpc/getsmartaccount) or AA SDK requires the usage of `createSessions` and `validateSession` (through the RPC endpoint you can access these through [createSessions](/aa/rpc/createsessions) and [validateSession](/aa/rpc/validatesession)). This functionality is only currently compatible with `BICONOMY` `version` 2.0.0, other smart account implementations won't work and will throw an error.
