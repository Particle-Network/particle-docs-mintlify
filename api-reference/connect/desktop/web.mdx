...
**Particle Connect** is an all-in-one SDK capable of handling end-to-end onboarding and wallet connection.
...
Below is an example using **yarn**.
...
Once **Particle Connect** is installed and you have your **project keys**, you can configure the SDK. Simply wrap your application in the `ConnectKitProvider` component to apply customizations and integrate the keys.
...
<Tip>Refer to the [Network Coverage](/guides/network-coverage) page for a detailed list.</Tip>
...
Moving out of your `index` file and into your main `App` component, you'll need to include the **Connect** button (`ConnectButton` from `@particle-network/connectkit`) to facilitate the useAccount of Particle Connect.
...
**Particle Connect** supports authentication with **Passkey**, a cutting-edge solution for secure, passwordless authentication. 

**Passkey** on **Connect** enables users to create non-custodial smart wallets by replacing traditional passwords with cryptographic key pairs. This delivers superior security, a streamlined user experience, and protection against phishing attacks.

To enable **Passkey** in your **Connect** project, import `passkeySmartWallet` from `"@particle-network/connectkit/evm"` and include it in the `evmWalletConnectors` object.

<Note>**Passkey** authentication is exclusively available with **Biconomy** or **Coinbase** smart accounts.</Note>
