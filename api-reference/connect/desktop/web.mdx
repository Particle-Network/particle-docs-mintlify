...
Particle Auth supports EVM and Solana chains. For a detailed list, please refer to [Network Coverage](/guides/network-coverage).

Import predefined chains, or you can also support other EVM-compatible chains by building your own chain object that inherits the `Chain` type.
...
You can customize the style of the Modal, including changing the accent color, border radius, primary button color, font, and more.
...
<Tip>
    Your custom `theme` will override the current mode's default values. If you have customized the background or
    foreground, it's recommended to specify the mode as either light or dark for a better experience.
</Tip>
...
<Note>
    For a complete list of supported chains for each aforementioned account implementation, head over to the [Network
    Coverage](/guides/network-coverage) page.
</Note>
...
When you enable **Account Abstraction** feature, you can send a `UserOperation` with `SmartAccount`.
