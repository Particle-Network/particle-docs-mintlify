...
Particle Connect supports numerous EVM chains alongside Solana. For a detailed list, please refer to [Network Coverage](/guides/network-coverage).
...
Particle Connect provides native support for handling and displaying information about the current chain being used. You can easily access these details by using the `chain` object, available through the `useAccount()` hook.
...
For Solana, `usePublicClient` returns a `Connection` type (imported from `@solana/web3.js`). Example Usage.
