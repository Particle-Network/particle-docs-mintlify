...
To begin, you'll need to install the Particle Connect Web SDK alongside viem.
...
Before jumping directly into configuration and implementation, you'll need to make sure that you've retrieved your `projectId`, `clientKey`, and `appId` from the [Particle dashboard](https://dashboard.particle.network). 

These are required values that you'll need in the coming configuration process, they'll authenticate your instance of Particle Connect.
...
-   Optionally, `plugins` — plugins enabling Particle's embedded wallet interface or implement account abstraction.
-   Optionally, `appearance` — collection of deep customization options affecting the login modal.
...
<Warning>
    When using a framework that supports React Server Components, you will need to include the `"use client"` directive at the beginning of the file.
</Warning>
...
Particle Auth supports numerous EVM chains alongside Solana. For a detailed list, please refer to [Network Coverage](/guides/network-coverage).

Import predefined chains, or when using standard wallet connection (not using the embedded wallet or social login features), you can also support other EVM-compatible chains by building your own chain object that inherits the `Chain` type.
...
You can customize the style of the modal, including changing the accent color, border radius, primary button color, font, and more; all through the `appearance` property.

You can experience it in the [Particle Demo](https://demo.particle.network) and export the configuration.
...
<Tip>
    Your custom `theme` will override the current mode's default values (`light`, `dark`, `auto`). If you have customized the background or foreground, it's recommended to specify the mode as either light or dark for a better experience.
</Tip>
...
The `aa` plugin relies on [Particle's Account Abstraction SDK](/api-reference/aa/sdks/desktop/web) to enable rich, built-in utilization of ERC-4337.
...
When you enable account abstraction, you'll need to handle the resulting account differently than you would normally; specifically, you'll need to use an instance of the `useSmartAccount` hook for constructing and sending UserOperations (transactions); these methods are available directly on the `useSmartAccount` instance.
...
<Note>
For more information about working with Particle Network's account abstraction stack, head over to the relevant [SDK reference](/api-reference/aa/sdks/desktop/web).
</Note>
...
<Warning>
How to get a smart account's address?

You can use `useAddress` imported from `particle-network/connectkit`.
...
When calling switch network, the SDK will either request the user to confirm the network switch or add the network if it was not previously set.
...
Particle Connect provides native support for handling and displaying blockchain information. You can easily access the details of the wallet's current active chain using the `chain` object, available through the `useAccount()` hook.
...
This structure enables you to dynamically access a wide range of blockchain details, offering flexibility in how you display and use this information within your application, thereby eliminating the need to hardcode blockchain-specific data.
...
Example of sending an Solana transaction:
