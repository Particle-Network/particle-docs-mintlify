...
| `rpcUrl` | `RpcUrl?` | Customize rpc url   |
...
`ParticleConnect` includes `ParticleAuthCore`. If you log in with a Particle account, you can access additional function such as `getUserInfo`, `openAccountAndSecurity`, `hasMasterPassword` and `hasPaymentPassword` and `changeMasterPassword`, you can reference the docs from [Auth SDK reference](/api-reference/auth/mobile/unity.mdx#get-user-info)