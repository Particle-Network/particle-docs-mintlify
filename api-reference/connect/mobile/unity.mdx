...
Once you've ensured that your project meets the above prerequisites, you'll also need three critical values from the [Particle dashboard](https://dashboard.particle.network): your `projectId`, `clientKey`, and `appId`.
...
2. Select `Other` from here, then scroll down to `Configuration`.
