...
The Particle Connect Flutter SDK is also largely open-source through the [GitHub repository](https://github.com/Particle-Network/particle-flutter). Before diving into the setup process below, it may help to build some context by diving into the underlying architecture and demos provided within [this repository](https://github.com/Particle-Network/particle-flutter/tree/master/particle-connect).
...
<Note>
  ...
  [Create a new
  project](/guides/wallet-as-a-service/waas/connect/flutter-quickstart).
</Note>
...
Now that you've spun up a project on the [Particle dashboard]() and configured your application, you're ready to move on to the initialization of the SDK itself. This must happen before the other methods become functional. To begin, you'll need to call `ParticleInfo.set`, passing in your `projectId` and client key (`clientKey`). Next, initialization can happen through the `init` function on `ParticleConnect`, which can be imported from `package:particle_connect/particle_connect.dart`. `init` takes the following parameters:
...
| `env`          | `Env`           | The specific environmennt to be used. This can be either `.dev`, `.staging`, `.production`, this should be set to `.production` when release                                                                                                                                                                                                                                                                                            |
| `rpcUrlConfig` | `RpcUrlConfig?` | Customize rpc url                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
`ParticleConnect.connectWithConnectKitConfig` is one click login function, which can present a customizable login UI. The parameter config can be derived from `ConnectKitConfig`.
...
| `socialProviders`         | `List<EnableSocialProvider>?` | Social providers, support `GOOGLE`, `APPLE` and other social options, the sort order is used for connect kit login UI.   |
| `walletProviders`         | `List<EnableWalletProvider>?` | Wallet providers, support `MetaMask`, `Trust` and other wallet options, the sort order is used for connect kit login UI. |
| `additionalLayoutOptions` | `AdditionalLayoutOptions`     | Layout options                                                                                                           |
| `logo`                    | `String`                      | logo url or base64 image                                                                                                 |
...
Set unsupported countries, used with phone login UI.
