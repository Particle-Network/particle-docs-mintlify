...
Now that you've created a project and an application within the [Particle dashboard](https://dashboard.particle.network), you'll need to go ahead and install the Particle Wallet SDK using CocoaPads (assuming you meet the minimum version requirements).
...
|--------|--------|----------------------------------------------------------------------------------------------------------|
| `from` | `String` | user public address, which to sign the transaction. |
...
| `action` | `Action` | the sepcific how to send this transaction. This can be either `.normal`, `.cancel`, `.speedUp`, defualt is `.normal`, which means send a new transaction|


<CodeGroup>

```swift Text Send native
...
| `methodName` | `String` | method name in the contract. |
| `params` | `[Encodable]` | the parameters for the method, each parameter requires a hexadecimal string. |
...
- `amount`, the volume of NFTs that you'd like to send, for NFT 721, the amount shoule always be 1.
