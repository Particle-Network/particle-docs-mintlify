...
<Note> `ParticleWalletGUI` is drived by adapters, `ParticleConnect.initialize` set a lot adapters, not you can get them and set to `ParticleWalletGUI`</Note>

Set adapters to driver
```swift
let adapters = ParticleConnect.getAdaptrs()
