...
| `icon` | `String` | a link to an icon/logo representing your project - ideally 512x512.  |
| `url` | `String` | the URL for your project's website. |
| `description` | `String` | a short description of your project.  |
| `walletConnectProjectId`| `String` | Wallet connect project id, get it from [WalletConnect dashboard](https://cloud.walletconnect.com) | 
...
| `amount` | `String` | the volume of tokens (`tokenAddress`) to be sent. |
...
| Field  | Type   | Description                                                                                              |
|--------|--------|----------------------------------------------------------------------------------------------------------|
| `walletAddress` | `String?` | the wallet address to receive the cryptocurrency. |
| `cryptoCoin` | `String?` |  cryptocurrency denomination.   |
| `fiatCoin` | `String?` | fiat currency denomination.   |
| `fiatAmt` | `String?` | the amount of fiat to be automatically filled in as the purchase volume.|
| `chainInfo` | `ChainInfo?` | the chainInfo|
| `fixFiatCoin` | `bool` | lock selection of fiat coin in the buy menu|
| `fixCryptoCoin` | `bool` | lock selection of fiat amount in the buy menu. |
| `theme` | `Theme` | the buy page theme, `dark` or `light`|
| `language` | `Language` | the buy page lanuage|
| `modalStyle` | `String` | control iOS presentation style, default is pageSheet|
