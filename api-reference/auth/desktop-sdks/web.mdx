...
To enable blind signatures (signing without a popup) in Particle Auth, certain conditions must be met when configuring the `AuthCoreContextProvider` component and setting up the authentication flow.
...
2. **No Payment Password**
   - Users should not have set a payment password.
...
4. **Use JWT or Telegram for Login**
   - User authentication flow is either:
     - JSON Web Token (JWT), or
     - Telegram.
