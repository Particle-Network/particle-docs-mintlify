...
Finally, you'll need to edit your Podfile to ensure `particle_auth_core` is properly imported. Head over to the [linked guide](https://github.com/Particle-Network/particle-react-native/blob/master/particle-auth-core/example/ios/Podfile) to complete this, if you haven't already.

<Note>
<p>Another important note before continuing.</p>
<p>Our SDK is a static library (XCFramework). When using the Particle Auth React Native SDK, you'll need to specify that you're using a static framework through the following:</p>
<p>
...
<Tip>You can use your existing user base or authentication method with Particle Connect through JWT. This way, your users can still log in with their current accounts. Click [here](/guides/configuration/auth/jwt) to learn how to configure jwt. </Tip>
...
// Email or phone login, through a presentation page.
...
// Here's how to log in to a custom page
...
// login with email or phone, the phone number must be in E.164 format.
final userInfo = await ParticleAuthCore.connectWithCode(null, "user@example.com", code);
final userInfo = await ParticleAuthCore.connectWithCode("+11234567890", null, this.state.code);
```
