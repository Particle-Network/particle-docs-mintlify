...
- `evm.signTypedData/evm.signTypeDataUnique`:  Requests a signature for structured (typed) data --`eth_signTypedData`. For more information on `signTypedData`, see the [Web SDK reference](/api-reference/auth/desktop-sdks/web).
...
<Tip>For specific details regarding custom style configurations, see the [Web](/api-reference/wallet/desktop/web) reference page.</Tip>
