...
- **Master Password**: A login-based non-recoverable password for securing access.
- **Payment Password**: A PIN required for transaction signatures.

These options ensure enhanced security for user interactions.
...
To obtain the balance of a specific token at a specific address, call `SolanaService.getTokenByTokenAddresses`
