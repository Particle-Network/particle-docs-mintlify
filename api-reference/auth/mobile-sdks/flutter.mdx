...
To throw a standard message for signature (meaning a user will be prompted in-UI to sign a given string), on Evm, you can call `Evm.personalSign`,  this method requires a hexadecimal string, on Solana, you can call `Solana.signMessage` you can pass in a UTF-8/readable string.
...
| Field                                  | Type  | Description                                     |
| -------------------------------------- | ----- | ----------------------------------------------- |
| `promptSettingWhenSign`                | `int` | security account config prompts (default is 1). |
| `promptMasterPasswordSettingWhenLogin` | `int` | master password prompts (default is 0).         |
...
| `gasFeeLevel`     | `GasFeeLevel`  | the gas fee level, `high`, `medium` or `low`, default is `high`.           |
