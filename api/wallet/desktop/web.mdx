...
Initializing Particle Wallet will require three key values, these will be your `projectId`, `clientKey`, and `appId`. All of Particle Network's SDKs require these values for API authentication, and thus they're easily accessible through the [Particle dashboard](https://particle-docs-demo.replit.app/). To configure your dashboard and therefore retrieve these values, do the following:
...
- Optionally, `customStyle`, for deeply customizing the styling (coloring, border weight, etc.) of the wallet modal. For more information on this, take a look at [Customizing Particle Wallet](doc:particle-wallet-customization).
...
2. `openWallet`, an alternative to `walletEntryCreate` that directly opens the wallet modal in fullscreen upon calling.  This takes various additional parameters, which you can learn more about at [Customizing Particle Wallet](doc:particle-wallet-customization).
