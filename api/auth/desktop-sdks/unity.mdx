...
The structure of this SDK follows the [Web (JavaScript/TypeScript)](/api/auth/desktop-sdks/web) page closely, albeit with several deviations, **as described below**.
...
Among these parameters, the `ParticleUnityRpc` prefab containing `project id`, `app id` and `client key` are the most important. It directly connects your instance of Particle's Wallet-as-a-Service with your configurations in the [Particle dashboard](https://dashboard.particle.network/). You can retrieve the values for these three parameters through the following:
...
`ParticleSystem.Instance.SignTypedData` facilitates the signature of structured data; by default, this uses `eth_signTypedData_v4`. For more information on `signTypedData`, see the [Web (JavaScript/TypeScript)](/api/auth/desktop-sdks/web) page.
