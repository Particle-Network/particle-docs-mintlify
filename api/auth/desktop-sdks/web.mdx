...
Integrating and leveraging Particle Auth isn't only bound to interfaces provided by Particle Network (such as [Particle Connect](doc:particle-connect)), rather it's built to fit into nearly every implementation scenario. Whether you're using RainbowKit, Web3Modal, Solana's `wallet-adapter`, or would instead like to have a standalone social login button within your application, Particle Auth can be used. Below you'll find an example of using Particle Auth within RainbowKit, one of the core connection kits officially supported by Particle.
