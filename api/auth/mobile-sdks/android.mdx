...
- `signTypedData`: **EVM-specific**, requests a signature for structured (typed) data- `eth_signTypedData`. For more information on `signTypedData`, see the [Web (JavaScript/TypeScript)](ref:auth-web) page.
...
> For specific details regarding custom style configurations, see the [Web](ref:web-1) Wallet reference.
