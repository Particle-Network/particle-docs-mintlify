...
Particle Auth also extends to Cocos with the Particle Auth Cocos SDK, facilitating complete integration of Particle's Wallet-as-a-Service within applications building games on Cocos. The utilization of Particle Auth and Cocos is virtually identical in nature to [React Native (JavaScript)](/api/auth/mobile-sdks/react), although the primary differentiator is in prerequisites and initial setup.

This document will outline the setup process; for examples of utilization, see the [React Native (JavaScript)](/api/auth/mobile-sdks/react).
...
<img
  ...
  src="/api/auth/mobile-sdks/images/cocos.png"
...
Using the Particle Auth Cocos SDK is identical to the examples showcased within the [React Native (JavaScript)](/api/auth/mobile-sdks/react). The only deviation is the import statement, which should contain an internal path, `../../Core/particleAuth`, as shown below:
