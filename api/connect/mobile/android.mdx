...
All implementations of Particle Connect (and Particle Auth) require three universal values: `projectId`, `appId`, and `clientKey`. These values tie a given implementation to the [Particle dashboard](<>), enabling configuration, analytics, tracking, etc. To retrieve these values (and use them in the next step), follow the below process:
