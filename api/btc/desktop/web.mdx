...
<Tip>To learn more about BTC Connect, see [Introduction to BTC Connect](ref:introduction-to-btc-connect) or take a look at the [GitHub repository](https://github.com/Particle-Network/particle-btc-connect).</Tip>

***

## Getting Started

BTC Connect follows [Particle Network's AA SDK](ref:aa-web) and [Particle Auth Core](ref:auth-web) very closely in terms of structure, so if you've used either of these SDKs before, you may find this process familiar. 
...
- `chainId`, the ID for the currently connected chain.
  - For a complete list of networks supported within BTC Connect, see [Network Coverage](doc:network-coverage).
...
- `getFeeQuotes`, for retrieving fee quotes (a collection of UserOperation structures based upon different fee payment mechanisms); this is equivalent to [getFeeQuotes](ref:getfeequotes).
