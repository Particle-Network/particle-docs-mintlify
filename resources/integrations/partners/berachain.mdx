...
Before constructing `AuthCoreContextProvider`, you'll need three values to initialize Particle Auth Core. When configuring any SDK from Particle Network, whether it's [Particle Auth Core](/api/auth/desktop-sdks/web), [Particle Connect](/api/connect/desktop/web), or [Particle's AA SDK](/api/aa/sdks/desktop/web), you'll need to retrieve a project ID, client key, and app ID from the [Particle dashboard](https://dashboard.particle.network). These values link your application with your project on the Particle dashboard, therefore authenticating API requests, tracking users, allowing for no-code modal customization, etc.
...
Take a look at the complete GitHub repository for the demo application partially showcased here. To learn more about Particle Auth Core, head over to [the Particle Auth SDK reference](/api/auth/desktop-sdks/web).
