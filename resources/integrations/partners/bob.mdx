...
- `aaOptions`, which contains `accountContracts`, taking:
  - `BTC`, the smart account implementation you'll be leveraging.
    ...
    - `version`, the version of the BTC smart account you intend to use. For BOB, this should be **`2.0.0`**. If you intend to use more than just BOB, you may need to define another instance of `BTC` using `version` `1.0.0`. For more information on which chains support `1.0.0` and which support `2.0.0`, head over to [Particle Network's documentation](/resources/quickstart/network-coverage#btc-connect).
