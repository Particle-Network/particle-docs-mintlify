...
Particle Network has natively implemented session keys through both the [Account Abstraction RPC](ref:aa-rpc-api) and the AA SDKs ([Introduction to Smart Wallet-as-a-Service](ref:introduction-to-mpc-aa)), allowing developers to begin using session keys directly within the Particle Network tech stack.

***

## Creating a session key

Creating a session key within Particle Network's [Account Abstraction RPC](ref:aa-rpc-api) or AA SDK requires the usage of `createSessions` and `validateSession` (through the RPC endpoint you can access these through [createSessions](ref:particle_aa_createsessions) and [validateSession](ref:particle_aa_validatesession)). This functionality is only currently compatible with `BICONOMY` `version` 2.0.0, other smart account implementations won't work and will throw an error.
