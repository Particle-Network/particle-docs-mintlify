...
<Tip>Particle Network's Omnichain Paymaster can be leveraged either through Particle's AA SDK ([Web (Desktop) Quickstart](doc:aa-web-quickstart) & [Mobile (Android/iOS) Quickstart](doc:mobile-aa-quickstart)) or the [Paymaster RPC](ref:paymaster-rpc).</Tip>