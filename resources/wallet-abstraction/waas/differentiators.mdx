...
<Note>Note: This chapter refers only to Particle's live [Modular Smart WaaS](/resources/wallet-abstraction/waas/overview) stack. As we release additional components, it'll be expanded to cover their unique aspects too. As a first-of-its-kind product, [BTC Connect](/resources/wallet-abstraction/btc-connect) is not covered here either.</Note>
...
<Tip>For a deeper dive into Particle's MPC-TSS solution, [click here](/resources/wallet-abstraction/waas/mpc-tss).</Tip>
...
- **Paymasters:** One of the prominent use cases of AA is the ability for dApps to sponsor gas fees for users, which Paymasters enable. This makes particular sense in solutions hosted in L2s, which typically have low costs for users. Paymasters can allow your dApp to accept gas fee payments in any native (not bridged) token, preventing users from constantly needing to acquire gas tokens. Particle has built a dedicated [Omnichain Paymaster](/resources/wallet-abstraction/waas/omni-paymaster) as a part of its AA stack. 
- **Bundlers:** Bundlers aggregate user actions for efficient blockchain interactions, potentially leading to reduced transaction fees and faster transaction confirmations​. Bundlers can result in better cost-efficiency for end-users and better performance for developers. Particle has also developed [an open-source Bundler](/resources/wallet-abstraction/waas/bundler), natively integrated into our AA stack.
