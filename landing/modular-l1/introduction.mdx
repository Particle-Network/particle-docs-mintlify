...
Modular Nodes validate and compute most key functionalities of Particle Network. They can be run by anyone that wants to support the Particle ecosystem’s decentralization.

These functionalities, as well as the different kinds of nodes performing them, are:
...
In addition to this, Relayer Nodes work alongside Particle Network’s Master Keystore Hub module to facilitate the automatic maintenance and parity of changes in smart account state across chains, updating account modules, owners, etc.

### Node monitoring: Watchtower Nodes

Given the various interconnected nodes simultaneously working alongside one another to power Universal Liquidity and Universal Accounts, a mechanism to monitor and ensure liveness among them is needed. Watchtower Nodes fulfill this need by acting as liveness assurance, monitoring the work of all other nodes in the network.
