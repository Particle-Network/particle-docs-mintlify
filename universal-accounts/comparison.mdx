...
description: "How Universal Accounts stack up against other cross-chain solutions."
---
...
## At a Glance
  <Card title="Universal Accounts" icon="atom-simple">
    **Account-level abstraction**
    ...
    One account + one balance. Cross-chain, gas, and routing are always on by default.
  </Card>
<CardGroup cols={3}>
<Card title="Avail Nexus" icon="diagram-project">
  **Intent-based interoperability**
  
  Describe desired outcomes while Nexus coordinates routing, execution, gas, and settlement.
</Card>
...
<Tabs>
  <Tab title="vs Relay">
    ...
    | **Setup required** | None | High configuration workload |
  </Tab>
...
| --- | --- | --- | --- | --- |
| **Base → Arbitrum** | ~1,605 ms | ~2,528 ms | ~2,982 ms | ~20s |
| **Base → Solana** | ~578 ms | ~3,360 ms | ~2,803 ms | N/A |
| **Solana → Base** | ~1,348 ms | — | ~2,803 ms | N/A |
...
<CardGroup cols={2}>
  ...
  <Card title="Consider alternatives if you need..." icon="circle-info">
    ...
    - **OneBalance**: Existing ERC-4337 smart account infrastructure
