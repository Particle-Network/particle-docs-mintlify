...
<Info>
  The following examples use a React-based app alongside [**Particle Auth**](/api-reference/auth/introduction) for authentication and wallet connection. However, the **Universal Accounts SDK is provider-agnostic**—alongside it, you can also use [**Particle Connect**](/api-reference/connect/introduction), **Web3Modal**, **RainbowKit**, or any **signer**. You can also use the SDK to construct and sign transactions programmatically on the server side.
</Info>
